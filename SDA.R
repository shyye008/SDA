...
error<-rnorm(n)
y<-sin(X[,s1]%*%b1)*exp(X[,s1]%*%b1)+error

SDA(H,y,X,1:5,method="Chi")
SDA(H,y,X,1:5,method="CvM")
SDA(H,y,X,1:5,method="KS")
...
error<-rnorm(n)
y<-sin(X[,s1]%*%b1)*exp(X[,s1]%*%b1)+0.5*error

MH<-SDA_knock(y,X,H,method="Chi",feature="Diff",index=c(1:100))
MH[[2]]
MH2<-SDA_knock(y,X,H,method="Chi",feature="SM",index=c(1:100))
MH2[[2]]
MH3<-SDA_knock(y,X,H,method="CvM",feature="SM",index=c(1:100),q=0.15)
MH3[[2]]
MH4<-SDA_knock(y,X,H,method="KS",feature="Diff",index=c(1:100))
MH4[[2]]
